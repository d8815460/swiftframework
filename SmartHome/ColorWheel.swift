...
class ColorWheel: UIView {
    ...
    // Overlay layer for the saturation
    var saturationLayer: CAShapeLayer!
    ...
    var indicatorColor: CGColorRef = UIColor.whiteColor().CGColor
    ...
    init(frame: CGRect, color: UIColor!) {
        ...
        // Layer for the Saturation
        saturationLayer = CAShapeLayer()
        saturationLayer.path = UIBezierPath(roundedRect: CGRect(x: 20.5, y: 20.5, width: self.frame.width-40.5, height: self.frame.height-40.5), cornerRadius: (self.frame.height-40.5)/2).CGPath
        self.layer.addSublayer(saturationLayer)
    ...
    func getIndicatorCoordinate(coord: CGPoint) -> (point: CGPoint, isCenter: Bool) {
        ...
        if (distance > radius) {
            ...
            outputCoord.x = ( radius - 20 ) * cos(theta) + wheelLayerCenter.x
            outputCoord.y = ( radius - 20 ) * sin(theta) + wheelLayerCenter.y
        }
    ...
    func setViewColor(color: UIColor!) {
        ...
        saturationLayer.fillColor = UIColor(white: 1, alpha: 1.0-self.saturation).CGColor
        point = pointAtHueSaturation(hue, saturation: saturation)
        drawIndicator()
    }
    
    func setViewSaturation(_saturation: CGFloat) {
        ...
        saturationLayer.fillColor = UIColor(white: 1, alpha: 1.0-self.saturation).CGColor
        self.color = UIColor(hue: hue, saturation: _saturation, brightness: brightness, alpha: 1.0)
        drawIndicator()
    }
