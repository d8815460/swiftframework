...
@objc protocol DrapDropCollectionViewDelegate {
    ...
    optional func dragDropCollectionViewDraggingDidEndForCellAtIndexPath(indexPath: NSIndexPath, toNewIndexPath newIndexPath: NSIndexPath)
}
...
class DragDropCollectionView: UICollectionView, UIGestureRecognizerDelegate {
    ...
    private var toGroupIndex: NSIndexPath?
        ...
        case UIGestureRecognizerState.Changed:
            ...
            if draggedCellIndexPath != nil {
                ...
                dispatchOnMainQueueAfterDelay(pingInterval, closure: { () -> () in
                    ...
                    self.toGroupIndex = shouldSwapCellsTuple.newIndexPath
                    
                    // 如果座標在中心範圍，就跳出詢問是否合併，如果不在中心範圍就變換位置
                    if (shouldSwapCellsTuple.newIndexPath != nil) {
                        let cell: UICollectionViewCell = self.dequeueReusableCellWithReuseIdentifier("SimpleCell", forIndexPath: shouldSwapCellsTuple.newIndexPath!) as! SimpleCell
                        
                        if (cell.frame.origin.x + 25 < touchLocation.x) && (touchLocation.x < cell.frame.origin.x + 85) && (cell.frame.origin.y + 25 < touchLocation.y) && (touchLocation.y < cell.frame.origin.y + 85){
                            print("停在中心點")
...
//                draggingDelegate?.dragDropCollectionViewDraggingDidEndForCellAtIndexPath!(draggedCellIndexPath!, toNewIndexPath: toGroupIndex!)
                if toGroupIndex != nil {
                    self.swapDraggedCellToGroupIndexPath(toGroupIndex!)
                }
    ...
    private func swapDraggedCellToGroupIndexPath(newIndexPath: NSIndexPath) {
        self.draggingDelegate?.dragDropCollectionViewDraggingDidEndForCellAtIndexPath!(self.draggedCellIndexPath!, toNewIndexPath: newIndexPath)
    }

    
    private func swapDraggedCellWithCellAtIndexPath(newIndexPath: NSIndexPath) {
        if self.draggedCellIndexPath?.section == newIndexPath.section {
            self.moveItemAtIndexPath(self.draggedCellIndexPath!, toIndexPath: newIndexPath)
        } else {
//            self.moveSection(self.draggedCellIndexPath!.section, toSection: newIndexPath.section)
        }
...
extension DragDropCollectionView {
    ...
    private func autoScroll(direction: AutoScrollDirection) {
        ...
        if ((direction == AutoScrollDirection.TowardsOrigin && newContentOffset.y < 0) || (direction == AutoScrollDirection.AwayFromOrigin && newContentOffset.y > self.contentSize.height - self.frame.height)) {
            ...
            dispatchOnMainQueueAfterDelay(0.3, closure: { 
            ...
            UIView.animateWithDuration(0.3
                ...
                , animations: { () -> Void in
                    ...
                    dispatchOnMainQueueAfterDelay(0.0, closure: { 
    ...
    internal func shouldAutoScroll(previousTouchLocation: CGPoint) -> (shouldScroll: Bool, direction: AutoScrollDirection) {
