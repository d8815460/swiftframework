...
//  Copyright © 2016年 kspri. All rights reserved.
//
...
class SaturationView: UIView {
    ...
    var indicatorBorderWidth: CGFloat = 4.0
    ...
    func touchHandler(touches: Set<UITouch>) {
        ...
        // Notify delegate of the new saturation
        delegate?.saturationSelected(getSaturationFromPoint())
        delegate?.saturation = self.getSaturationFromPoint()
        drawIndicator()
    }
    ...
    func getSaturationFromPoint() -> CGFloat {
        // Get the saturation value for a given point
    ...
    func getPointFromColor(color: UIColor) -> CGPoint {
        ...
        return CGPoint(x: saturation * frame.width, y: frame.height / 2)
    }
